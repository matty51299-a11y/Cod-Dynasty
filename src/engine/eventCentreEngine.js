...
export function makeTransferOfferEvent(offer, player, state) {
  ...
  return makeEvent({
    ...
    summary: `${buyer?.name ?? "A pro team"} have submitted a buyout offer for ${player?.name ?? "your player"}.`,
    season: state.season,
...
export function makeChallengerBuyoutEvent(offer, state) {
  ...
  return makeEvent({
    ...
    category: "Transfers",
    severity: "high",
    title: `${buyer?.tag ?? "A pro team"} bid ${fmtFee(offer.fee)} for ${offer.playerName}`,
    summary: `${buyer?.name ?? "A pro team"} want to sign ${offer.playerName} from your Open Circuit roster.`,
    season: state.season,
    stage: state.schedule?.stageIdx ?? 0,
    phase: state.schedule?.phase ?? "stage",
    actionRequired: true,
...
export function makeQualifyForChampsEvent(state) {
  return makeEvent({
    ...
    summary: "Your team has qualified for the Championship. Prepare for the biggest event of the season.",
    season: state.season,
