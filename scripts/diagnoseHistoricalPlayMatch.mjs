...
check("No duplicate players appear in K/D tables", new Set([...live.mapResults[0].playerStats.teamA, ...live.mapResults[0].playerStats.teamB].map(p => p.playerId)).size === 8);
...
check("Event results are not duplicated", applied.matches.filter(m => m.id === userMatch.id && m.status === "completed").length === 1);
