...
for (const important of ["TeePee","Karma","Crimsix","ACHES"]) { const active = aw.players.filter(p=>p.teamId&&p.name.toLowerCase()===important.toLowerCase()); check(`${important} is active on one team only`, active.length===1, active.map(p=>p.teamId).join(",")); }
...
check("No missing_error rows in transition audit", !rows.some(r=>r[4]==="missing_error"));
