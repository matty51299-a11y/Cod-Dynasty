...
check("Event has all active pro teams entered", state.field.length === GHOSTS_TEAMS.length && state.field.some(t => t.teamId === userTeamId));
...
check("CoD Champs Online Qualifier opens with every team and matchups", qualifierState.format === "single_elimination" && qualifierState.field.length === GHOSTS_TEAMS.length && qualifierState.matches.length > 0);
...
check("Save/load preserves completed event detail state", saved.activeEventId === event.id && saved.eventProgress[event.id].matches.length === state.matches.length && saved.eventProgress[event.id].champion.teamId === state.champion.teamId);
...
check("Event UI has current matchup section", eventDetailSource.includes("Current Matches") && eventDetailSource.includes("MatchCard"));
